...
- `tools/wasmtime_full_backend_matrix.py`: Compare Wasmtime with the retained `uwvm-int-full` and LLVM AOT/full execution modes over a WAT corpus.
...
includes the `ros` product profile plus chunked SHA-256/size fingerprints for
...
unallowlistable `wat-compile-skipped`; stale output is never reused. Equal
timeouts, signals, and unclassified runtime errors are operational failures
rather than matching results. In ROS, `-Rint` selects the retained full
interpreter backend; the driver intentionally has no lazy or tiered profiles
because those modes are not part of this reduced runtime.
