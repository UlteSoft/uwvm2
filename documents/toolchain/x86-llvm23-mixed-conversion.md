...
# ROS stable-vendor update (2026-09-16)

ROS now pins the official LLVM 23.1.1 release, whose `lowerFPToIntToFP`
tracks the two conversions' signedness independently. The raw/unconstrained
regression passed 1,876 exact-bit checks across seven x86 sub-ISA configurations
at O0/O3, including AVX512 without VL and i686/x86-64 x87. ROS therefore uses
direct integer-to-float emission again; its pinned-version header rejects the
old development toolchain. Ordinary uwvm2 retains the compatibility workaround
because it still allows other LLVM installations. See
[vendor provenance and build policy](../../third-parties/llvm/README.md).
The historical diagnosis and earlier snapshot results above remain relevant
when auditing an older LLVM; they are not a claim that stable 23.1.1 has the bug.
