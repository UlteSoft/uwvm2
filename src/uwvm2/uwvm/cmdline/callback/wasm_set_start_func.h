...
UWVM_MODULE_EXPORT namespace uwvm2::uwvm::cmdline::params::details
{
    ...
    template <typename T>
    [[nodiscard]] inline bool wasm_set_start_func_parse_hexfloat_range(char8_t const* first, char8_t const* last, T& value) noexcept
    {
        if(first == last) { return false; }
        ...
        while(first != last)
        {
            ...
            if(!wasm_set_start_func_hex_digit(*first, digit)) { break; }
...
#endif
        return wasm_set_start_func_parse_hexfloat_range(first, last, value);
    }
