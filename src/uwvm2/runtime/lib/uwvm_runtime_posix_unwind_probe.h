...
// ensure_llvm_jit_native_target_initialized declaration. No platform register layout or logical Wasm stack is used here.
        ...
        [[nodiscard]] inline constexpr bool runtime_llvm_jit_posix_live_unwind_probe() noexcept
        {
            ...
            auto const root_type{::llvm::FunctionType::get(void_type, {ptr_type, ptr_type}, false)};
            auto const recursive_type{::llvm::FunctionType::get(void_type, {ptr_type, i32_type, ptr_type}, false)};
            ...
            // Pass the host callback as a C-ABI function pointer, not a named
            // external direct call. MIPS MCJIT's default static model can emit
            // a JAL via a far stub that sets $at rather than $t9, while a PIC
            // host callee derives GP from t9. Such a probe may fault before it can
            // decide whether native unwinding works. A genuine indirect C call keeps
            // that ABI setup (and other targets' descriptor rules) intact.
            // This affects only the one-time probe, not generated Wasm calls.
            builder.CreateCall(capture_type, recursive->getArg(2), {recursive->getArg(0)});
            builder.CreateRetVoid();
            builder.SetInsertPoint(recurse);
            builder.CreateCall(recursive, {recursive->getArg(0), builder.CreateSub(recursive->getArg(1), builder.getInt32(1)), recursive->getArg(2)});
            builder.CreateRetVoid();
            builder.SetInsertPoint(::llvm::BasicBlock::Create(context, "entry", root));
            builder.CreateCall(recursive, {root->getArg(0), builder.getInt32(2), root->getArg(1)});
            builder.CreateRetVoid();
