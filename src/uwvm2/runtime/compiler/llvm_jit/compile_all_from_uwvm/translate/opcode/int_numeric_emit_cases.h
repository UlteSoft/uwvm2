...
case wasm1_code::i32_clz:
{
    ...
    if(!emit_unary(runtime_operand_stack_value_type::i32,
                   ...
                   [&](llvm_jit_stack_value_t const& operand) noexcept {
...
case wasm1_code::i32_add:
{
    ...
    if(!emit_binary(runtime_operand_stack_value_type::i32,
                    runtime_operand_stack_value_type::i32,
                    [&](llvm_jit_stack_value_t const& left, llvm_jit_stack_value_t const& right) noexcept { return ir_builder.CreateAdd(left.value, right.value); }))
    {
        return result;
    }
    break;
...
case wasm1_code::i32_sub:
{
    ...
    if(!emit_binary(runtime_operand_stack_value_type::i32,
                    runtime_operand_stack_value_type::i32,
                    [&](llvm_jit_stack_value_t const& left, llvm_jit_stack_value_t const& right) noexcept { return ir_builder.CreateSub(left.value, right.value); }))
    {
        return result;
    }
    break;
...
case wasm1_code::i32_mul:
{
    ...
    if(!emit_binary(runtime_operand_stack_value_type::i32,
                    runtime_operand_stack_value_type::i32,
                    [&](llvm_jit_stack_value_t const& left, llvm_jit_stack_value_t const& right) noexcept { return ir_builder.CreateMul(left.value, right.value); }))
    {
        return result;
    }
    break;
...
case wasm1_code::i32_and:
{
    ...
    if(!emit_binary(runtime_operand_stack_value_type::i32,
                    runtime_operand_stack_value_type::i32,
                    [&](llvm_jit_stack_value_t const& left, llvm_jit_stack_value_t const& right) noexcept { return ir_builder.CreateAnd(left.value, right.value); }))
    {
        return result;
    }
    break;
...
case wasm1_code::i32_or:
{
    ...
    if(!emit_binary(runtime_operand_stack_value_type::i32,
                    runtime_operand_stack_value_type::i32,
                    [&](llvm_jit_stack_value_t const& left, llvm_jit_stack_value_t const& right) noexcept { return ir_builder.CreateOr(left.value, right.value); }))
    {
        return result;
    }
    break;
...
case wasm1_code::i32_xor:
{
    ...
    if(!emit_binary(runtime_operand_stack_value_type::i32,
                    runtime_operand_stack_value_type::i32,
                    [&](llvm_jit_stack_value_t const& left, llvm_jit_stack_value_t const& right) noexcept { return ir_builder.CreateXor(left.value, right.value); }))
    {
        return result;
    }
    break;
...
case wasm1_code::i32_shl:
{
    ...
    if(!emit_binary(runtime_operand_stack_value_type::i32,
                    runtime_operand_stack_value_type::i32,
                    [&](llvm_jit_stack_value_t const& left, llvm_jit_stack_value_t const& right) noexcept { return ir_builder.CreateShl(left.value, emit_llvm_shift_count_mask(ir_builder, right.value, get_llvm_integer_bit_width(left.value))); }))
    {
        return result;
    }
    break;
...
case wasm1_code::i32_shr_s:
{
    ...
    if(!emit_binary(runtime_operand_stack_value_type::i32,
                    runtime_operand_stack_value_type::i32,
                    [&](llvm_jit_stack_value_t const& left, llvm_jit_stack_value_t const& right) noexcept { return ir_builder.CreateAShr(left.value, emit_llvm_shift_count_mask(ir_builder, right.value, get_llvm_integer_bit_width(left.value))); }))
    {
        return result;
    }
    break;
...
case wasm1_code::i32_shr_u:
{
    ...
    if(!emit_binary(runtime_operand_stack_value_type::i32,
                    runtime_operand_stack_value_type::i32,
                    [&](llvm_jit_stack_value_t const& left, llvm_jit_stack_value_t const& right) noexcept { return ir_builder.CreateLShr(left.value, emit_llvm_shift_count_mask(ir_builder, right.value, get_llvm_integer_bit_width(left.value))); }))
    {
        return result;
    }
    break;
...
case wasm1_code::i32_rotl:
{
    ...
    if(!emit_binary(runtime_operand_stack_value_type::i32,
                    runtime_operand_stack_value_type::i32,
                    [&](llvm_jit_stack_value_t const& left, llvm_jit_stack_value_t const& right) noexcept { return emit_llvm_rotl(ir_builder, left.value, right.value); }))
    {
        return result;
    }
    break;
...
case wasm1_code::i32_rotr:
{
    ...
    if(!emit_binary(runtime_operand_stack_value_type::i32,
                    runtime_operand_stack_value_type::i32,
                    [&](llvm_jit_stack_value_t const& left, llvm_jit_stack_value_t const& right) noexcept { return emit_llvm_rotr(ir_builder, left.value, right.value); }))
    {
        return result;
    }
    break;
...
case wasm1_code::i32_div_s:
{
    ...
    if(!emit_binary(runtime_operand_stack_value_type::i32,
                    runtime_operand_stack_value_type::i32,
                    [&](llvm_jit_stack_value_t const& left, llvm_jit_stack_value_t const& right) noexcept {
...
case wasm1_code::i32_rem_s:
{
    ...
    if(!emit_binary(runtime_operand_stack_value_type::i32,
                    runtime_operand_stack_value_type::i32,
                    [&](llvm_jit_stack_value_t const& left, llvm_jit_stack_value_t const& right) noexcept { return emit_llvm_signed_remainder_with_wasm_semantics(*llvm_module, ir_builder, left.value, right.value); }))
    {
        return result;
    }
    break;
