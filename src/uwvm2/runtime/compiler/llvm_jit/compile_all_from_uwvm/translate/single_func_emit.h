                                                                                       ...
                                                                                       ::llvm::Type* pointer_type) noexcept
{
    ...
    for(unsigned shift{static_cast<unsigned>(sizeof(::std::uintptr_t) * 8u - 8u)};; shift -= 8u)
    {
        ...
        {
            address_value =
                ir_builder.CreateShl(address_value, ::llvm::ConstantInt::get(llvm_intptr_type, 8u), get_llvm_string_ref(u8"uwvm.riscv64.host.addr.shl"));
        }
...
{
    return ::uwvm2::utils::container::u8concat_uwvm(get_llvm_runtime_bridge_function_symbol_name<Function>(discriminator),
                                                   u8"_",
                                                                                  ...
                                                                                  ::llvm::IntegerType* llvm_intptr_type) noexcept
{
    ...
    // when lowering llvm.frameaddress in generated trap blocks. Pass zero and let the runtime use the instruction path.
    static_cast<void>(ir_builder);
