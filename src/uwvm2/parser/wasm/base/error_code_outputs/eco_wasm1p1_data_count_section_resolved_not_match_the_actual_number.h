...
if constexpr(::std::same_as<char_type, char>)
{
    wasm1p1_print_error("WebAssembly 1.1 data count section declares ",
                        errout.err.err_selectable.wasm1p1_data_count_mismatch.expected,
                        " data segments, but the data section contains ",
                        errout.err.err_selectable.wasm1p1_data_count_mismatch.actual,
                        ".");
}
else if constexpr(::std::same_as<char_type, wchar_t>)
{
    wasm1p1_print_error(L"WebAssembly 1.1 data count section declares ",
                        errout.err.err_selectable.wasm1p1_data_count_mismatch.expected,
                        L" data segments, but the data section contains ",
                        errout.err.err_selectable.wasm1p1_data_count_mismatch.actual,
                        L".");
}
else if constexpr(::std::same_as<char_type, char8_t>)
{
    wasm1p1_print_error(u8"WebAssembly 1.1 data count section declares ",
                        errout.err.err_selectable.wasm1p1_data_count_mismatch.expected,
                        u8" data segments, but the data section contains ",
                        errout.err.err_selectable.wasm1p1_data_count_mismatch.actual,
                        u8".");
}
else if constexpr(::std::same_as<char_type, char16_t>)
{
    wasm1p1_print_error(u"WebAssembly 1.1 data count section declares ",
                        errout.err.err_selectable.wasm1p1_data_count_mismatch.expected,
                        u" data segments, but the data section contains ",
                        errout.err.err_selectable.wasm1p1_data_count_mismatch.actual,
                        u".");
}
else if constexpr(::std::same_as<char_type, char32_t>)
