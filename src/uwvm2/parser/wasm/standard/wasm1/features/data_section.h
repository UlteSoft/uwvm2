...
UWVM_MODULE_EXPORT namespace uwvm2::parser::wasm::standard::wasm1::features
{
    ...
    namespace details::data_section_print
    {
        ...
        using body_wrapper_t = decltype(section_details(
            ::std::declval<data_section_storage_t<Fs...> const&>().datas.index_unchecked(0uz),
        ...
        UWVM_WASM_UTILS_DEFINE_CONTEXT_LITERAL(row_prefix, " - data[");
        ...
        struct context
        {
                ...
                char_type* end) noexcept
            {
                ...
                if(data_section_size == 0uz || this->curr_stage == stage::done) { return {curr, true}; }

                auto const& datas{data_section_details_wrapper.data_section_storage_ptr->datas};
                auto const data_size{datas.size()};
                ...
                for(;;)
                {
                    switch(this->curr_stage)
                    {
                        ...
                        {
                            if(!emit_reserve(curr, end, data_size, this->offset)) { return {curr, false}; }
                        ...
                        case stage::header_suffix:
                        {
                            ...
                            this->curr_stage = data_size == 0uz ? stage::done : stage::row_prefix;
                            break;
                        }
                        ...
                        {
                            if(!emit_reserve(curr, end, this->data_counter, this->offset)) { return {curr, false}; }
                        ...
                        case stage::row_body:
                        {
                            auto const& curr_data{datas.index_unchecked(this->data_counter)};
                            if(!emit_dynamic_reserve(curr, end, section_details(curr_data, *data_section_details_wrapper.all_sections_ptr), this->offset))
                            {
                                return {curr, false};
                            }
    ...
    template <::std::integral char_type, ::uwvm2::parser::wasm::concepts::wasm_feature... Fs>
        requires details::data_section_print::context_body_supported<char_type, Fs...>
    inline constexpr auto print_context_type(::fast_io::io_reserve_type_t<char_type, data_section_storage_section_details_wrapper_t<Fs...>>) noexcept
    { return ::fast_io::io_type_t<::uwvm2::parser::wasm::standard::wasm1::features::details::data_section_print::context>{}; }

    template <::std::integral char_type, ::uwvm2::parser::wasm::concepts::wasm_feature... Fs>
        requires details::data_section_print::context_body_supported<char_type, Fs...>
    inline constexpr ::std::size_t print_context_static_buffer_size(
        ::fast_io::io_reserve_type_t<char_type, data_section_storage_section_details_wrapper_t<Fs...>>) noexcept
    {
