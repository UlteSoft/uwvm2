...
UWVM_MODULE_EXPORT namespace uwvm2::parser::wasm::standard::wasm1p1::features
{
    ...
    /// @brief Parser-owned COP tag selecting the established WebAssembly 1.1 code-validation strategy.
    struct wasm1p1_code_version
    {};

    /// @brief Records which mutually-exclusive family of Wasm feature switches owns the command line.
    /// @details The three complete feature-set switches conflict with each other and with every scoped enable/disable switch.
    ...
    /// @details MVP funcref remains legal. The externref table form uses both the reference-types value space and the table-instructions group that
    /// generalized table element types.
    template <::uwvm2::parser::wasm::concepts::wasm_feature... Fs>
