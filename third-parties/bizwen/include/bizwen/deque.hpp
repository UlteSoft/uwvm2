...
class deque
{
    ...
    static constexpr bool is_default_destroy_ = !requires(Alloc &a) { a.destroy(static_cast<T *>(nullptr)); };
  ...
  ctrl_end   →
    */
    ...
    {
        if constexpr (!(::std::is_trivially_destructible_v<T> && is_default_destroy_))
        {
            // 4种情况，0，1，2，3+个块有元素
