...
bool SelectionDAGLegalize::ExpandNode(SDNode *Node) {
  ...
  case ISD::STRICT_FSETCCS: {
    ...
    EVT CompareVT = Tmp1.getValueType();
    SDValue Mask, EVL;
    if (IsVP) {
    ...
    if (Legalized) {
      ...
      if (NeedInvert) {
        // Boolean representation belongs to the compared operands, not the
        // integer result type. MIPS R6 FP comparisons produce 0/-1 while
        // integer comparisons produce 0/1. XORing an FP comparison with 1
        // turns true into -2, corrupting sign-extended SIMD predicate masks.
        // Keep the original operand type even if condition legalization
        // replaces the comparison with a composite boolean expression.
        EVT ResultVT = Tmp1.getValueType();
        SDValue TrueValue = DAG.getBoolConstant(true, dl, ResultVT, CompareVT);
        if (!IsVP)
          Tmp1 = DAG.getNode(ISD::XOR, dl, ResultVT, Tmp1, TrueValue);
        else
          Tmp1 = DAG.getNode(ISD::VP_XOR, dl, ResultVT, Tmp1, TrueValue, Mask, EVL);
      }
