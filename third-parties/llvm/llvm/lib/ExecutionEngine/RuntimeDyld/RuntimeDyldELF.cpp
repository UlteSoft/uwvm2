...
void RuntimeDyldELF::resolveAArch64Relocation(const SectionEntry &Section,
  ...
  case ELF::R_AARCH64_CONDBR19: {
    ...
    write32le(TargetPtr, read32le(TargetPtr) & 0xff00001fU);
  ...
  case ELF::R_AARCH64_LD_PREL_LO19: {
    ...
    // Instruction words are always LE, including a BE native JIT host.
    write32le(TargetPtr, read32le(TargetPtr) & 0xff00001fU);
    // Immediate goes in bits 23:5 of LD imm instruction, taken
    // from bits 20:2 of X
    // Preserve the full signed imm19 (offset bits 20:2), not only the low
    // ten bits. The old 0xffc mask silently truncated offsets beyond 4 KiB.
    or32le(TargetPtr, (Result & 0x1ffffc) << (5 - 2));
    break;
  }
  case ELF::R_AARCH64_ADR_PREL_LO21: {
    ...
    write32le(TargetPtr, read32le(TargetPtr) & 0x9f00001fU);
    // Immediate goes in bits 23:5, 30:29 of ADR imm instruction, taken
    // from bits 20:0 of X
    // ADR has a signed 21-bit byte offset split across immhi and immlo.
    // AAELF64 requires all bits 20:0; truncating immhi to 4 KiB changes the
    // referenced address even when the relocation's range check succeeds.
    or32le(TargetPtr, ((Result & 0x1ffffc) << (5 - 2)) |
                        ((Result & 0x3) << 29));
    break;
  }
...
RuntimeDyldELF::processRelocationRef(
  ...
  if (Symbol != Obj.symbol_end()) {
    ...
    if (Symbol->getBinding() == ELF::STB_LOCAL) {
      // Relocations identify an ELF symbol-table entry, not a unique name.
      // LLVM can emit multiple local ".L0 " entries in one object (notably
      // RISC-V/LoongArch EH-frame start labels). A name lookup aliases all
      // such entries to the last function and silently corrupts its FDEs.
      // Resolve defined locals from the exact referenced entry; keep ordinary
      // global/weak resolution and STN_UNDEF handling unchanged.
      ...
      if (*Flags & SymbolRef::SF_Absolute) {
        ...
        Value.SectionID = AbsoluteSymbolSection;
        Value.Offset = *Address;
        Value.Addend = *Address + Addend;
        ResolvedLocal = true;
      } else if (!(*Flags & SymbolRef::SF_Undefined)) {
        auto SI = Symbol->getSection();
        if (!SI)
          return SI.takeError();
        if (*SI != Obj.section_end()) {
          auto ID = findOrEmitSection(Obj, **SI, (*SI)->isText(), ObjSectionToID);
          if (!ID)
            return ID.takeError();
          auto Address = Symbol->getAddress();
          if (!Address)
            return Address.takeError();
          uint64_t SymbolOffset = *Address - (*SI)->getAddress();
          Value.SectionID = *ID;
          Value.Offset = SymbolOffset;
          Value.Addend = SymbolOffset + Addend;
          ResolvedLocal = true;
        }
      }
    }
  ...
  if (ResolvedLocal) {
    // Value already describes the specific local symbol-table entry.
  } else if (gsi != GlobalSymbolTable.end()) {
