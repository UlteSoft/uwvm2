...
uint8_t *RuntimeDyldImpl::createStubFunction(uint8_t *Addr,
  ...
  if (Arch == Triple::aarch64 || Arch == Triple::aarch64_be ||
    ...
    // AArch64 instruction bytes are little-endian even for big-endian ELF
    // data. writeBytesUnaligned follows the data byte order and would produce
    // invalid stub instructions on aarch64_be. The instruction relocations
    // below likewise operate on little-endian words, independently of host.
    support::endian::write32le(Addr,    0xd2e00010); // movz ip0, #:abs_g3:<addr>
    support::endian::write32le(Addr+4,  0xf2c00010); // movk ip0, #:abs_g2_nc:<addr>
    support::endian::write32le(Addr+8,  0xf2a00010); // movk ip0, #:abs_g1_nc:<addr>
    support::endian::write32le(Addr+12, 0xf2800010); // movk ip0, #:abs_g0_nc:<addr>
    support::endian::write32le(Addr+16, 0xd61f0200); // br ip0
