...
bool VEInstrInfo::expandExtendStackPseudo(MachineInstr &MI) const {
  ...
  // The split happens after register allocation: arguments still live in the
  // continuation must also be live through the optional stack-growth block.
  // Compute successor first so the syscall block sees the complete live-out.
  // Backport of LLVM PR #221553; this does not add runtime instructions.
