...
bool AVRShiftExpand::runOnFunction(Function &F) {
  ...
  SmallVector<BinaryOperator *, 1> VectorShifts;
  for (Instruction &I : instructions(F))
    if (I.isShift() && isa<FixedVectorType>(I.getType()))
      VectorShifts.push_back(cast<BinaryOperator>(&I));

  // AVR has no native vectors. Scalarize each lane before the scalar loop
  // expansion below: truncating a vector shift count to scalar i8 is invalid
  // IR, and skipping vectors would let wide variable shifts become unavailable
  // __ashlsi3-style libcalls during type legalization. Keep lane-specific
  // counts (including nonsplats), rather than assuming one count for a vector.
  for (BinaryOperator *I : VectorShifts) {
    IRBuilder<> B(I);
    auto *Ty = cast<FixedVectorType>(I->getType());
    Value *Result = PoisonValue::get(Ty);
    for (unsigned Lane = 0; Lane != Ty->getNumElements(); ++Lane) {
      Value *X = B.CreateExtractElement(I->getOperand(0), Lane);
      Value *Count = B.CreateExtractElement(I->getOperand(1), Lane);
      Value *Shift = B.CreateBinOp(I->getOpcode(), X, Count);
      Result = B.CreateInsertElement(Result, Shift, Lane);
    }
  ...
  return !ShiftInsts.empty() || !VectorShifts.empty();
}
