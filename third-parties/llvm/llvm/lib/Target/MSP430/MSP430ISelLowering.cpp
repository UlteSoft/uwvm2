...
MSP430TargetLowering::EmitShiftInstr(MachineInstr &MI,
  ...
  // As with select expansion, a variable shift can split argument setup in
  // an active call frame. Preserve the frame size on both new entry edges.
  unsigned CallFrameSize = TII.getCallFrameSizeAt(MI);
  LoopBB->setCallFrameSize(CallFrameSize);
  RemBB->setCallFrameSize(CallFrameSize);
...
MSP430TargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
  ...
  F->insert(I, copy0MBB);
  F->insert(I, copy1MBB);
  // A select may be scheduled between call-frame setup and argument stores.
  // Both sides of the diamond inherit that in-progress frame, not the default
  // zero. The sink may contain the matching ADJCALLSTACKUP. Do not disable the
  // verifier: later frame/SP reasoning also needs these entry sizes.
  unsigned CallFrameSize = TII.getCallFrameSizeAt(MI);
  copy0MBB->setCallFrameSize(CallFrameSize);
  copy1MBB->setCallFrameSize(CallFrameSize);
  // Update machine-CFG edges by transferring all successors of the current
