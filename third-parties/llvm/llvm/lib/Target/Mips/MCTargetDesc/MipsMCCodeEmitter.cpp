...
void MipsMCCodeEmitter::encodeInstruction(const MCInst &MI,
  ...
  if (isMicroMips(STI)) {
    if (isMips32r6(STI)) {
      ...
      // Private FGR64 low-word copy opcodes deliberately have no shared
      // TableGen mapping key with their FGR32 counterparts. They encode the
      // same register-transfer operation; leaving the standard MIPS word
      // unchanged in a microMIPS stream executes an unrelated instruction.
      if (Opcode == Mips::MFC1_D64)
        NewOpcode = Mips::MFC1_MMR6;
      else if (Opcode == Mips::MTC1_D64)
        NewOpcode = Mips::MTC1_MMR6;
      // Prologue insertion and physical register copies also bypass DAG
      // patterns. These standard opcodes lack a Std2MicroMipsR6 mapping;
      // encode the selected operation, never the standard-MIPS bit pattern.
      else if (Opcode == Mips::LUi || Opcode == Mips::LUi_MM)
        NewOpcode = Mips::LUI_MMR6;
      else if (Opcode == Mips::FMOV_S)
        NewOpcode = Mips::FMOV_S_MMR6;
      else if (Opcode == Mips::FMOV_D64)
        NewOpcode = Mips::FMOV_D_MMR6;
      // PseudoCVT expansion happens after selection and uses the private
      // standard opcodes too. Keep integer-to-float conversion in the chosen
      // instruction stream; an ordinary CVT word is not a microMIPS opcode.
      else if (Opcode == Mips::CVT_S_W)
        NewOpcode = Mips::CVT_S_W_MMR6;
      else if (Opcode == Mips::CVT_D64_W)
        NewOpcode = Mips::CVT_D64_W_MM;
      else if (Opcode == Mips::CVT_S_L)
        NewOpcode = Mips::CVT_S_L_MMR6;
      else if (Opcode == Mips::CVT_D64_L)
        NewOpcode = Mips::CVT_D_L_MMR6;
    }
    else
