...
MipsSETargetLowering::MipsSETargetLowering(const MipsTargetMachine &TM,
  ...
  if (Subtarget.hasMips32r6()) {
    ...
    // R6 strict comparisons use the chain-aware CMP patterns. The inherited
    // custom action uses pre-R6 FCC0/CMOV instructions, which do not exist on
    // this ISA. Predicate expansion still handles inversions/swapped operands
    // and must preserve R6's floating-point 0/-1 boolean representation.
    if (!Subtarget.useSoftFloat()) {
