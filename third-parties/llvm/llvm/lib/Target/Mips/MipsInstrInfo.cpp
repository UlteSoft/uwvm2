...
unsigned MipsInstrInfo::getEquivalentCompactForm(
  ...
  if (Subtarget.inMicroMipsMode()) {
    ...
    // of the operands is zero.
      // R6 uses a different compact-branch encoding. Do not select the
      // pre-R6 microMIPS BEQZC/BNEZC opcodes merely because an operand is zero;
      // the normal R6 case below is mapped to its microMIPS R6 counterpart.
      if (!Subtarget.hasMips32r6() &&
          I->getOperand(1).getReg() == Subtarget.getABI().GetZeroReg())
        canUseShortMicroMipsCTI = true;
      break;
    // Both microMIPS revisions have a short no-delay-slot register jump, but
    // their encodings differ. Select the matching revision below.
    case Mips::JR:
  ...
  if (Subtarget.hasMips32r6() || canUseShortMicroMipsCTI) {
    ...
    case Mips::TAILCALLR6REG:
      if (canUseShortMicroMipsCTI)
        return Subtarget.hasMips32r6() ? Mips::JRC16_MMR6 : Mips::JRC16_MM;
      return Mips::JIC;
