...
#include <uwvm2/imported/wasi/wasip1/func/sched_yield_wasm64.h>
...
int main()
{
    ...
    // Case 0: default implementation (no callback) should return esuccess
    {
        ...
        auto const err = ::uwvm2::imported::wasi::wasip1::func::sched_yield_wasm64(env);
        if(err != wasi_errno_t::esuccess)
        {
            ::fast_io::io::perrln(::fast_io::u8err(), u8"sched_yield_wasm64: default implementation failed");
    ...
    // Case 1: custom callback should be invoked and its errno propagated
    {
        ...
        {
            ::fast_io::io::perrln(::fast_io::u8err(), u8"sched_yield_wasm64: callback not invoked");
            ::fast_io::fast_terminate();
        }

        if(err != g_next_return_value)
        {
            ::fast_io::io::perrln(::fast_io::u8err(), u8"sched_yield_wasm64: errno mismatch");
            ::fast_io::fast_terminate();
        }
    }
}
