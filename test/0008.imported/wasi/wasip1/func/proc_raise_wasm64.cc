...
int main()
{
    ...
    if(!g_raise_called)
    {
        ::fast_io::io::perrln(::fast_io::u8err(), u8"proc_raise_wasm64: callback not invoked");
        ::fast_io::fast_terminate();
    }

    if(g_last_signal != static_cast<wasm_i32>(signal_code))
    {
        ::fast_io::io::perrln(::fast_io::u8err(), u8"proc_raise_wasm64: signal code mismatch");
        ::fast_io::fast_terminate();
    }

    if(err != static_cast<errno_t>(g_next_return_value))
    {
        ::fast_io::io::perrln(::fast_io::u8err(), u8"proc_raise_wasm64: errno mismatch");
        ::fast_io::fast_terminate();
    }
}
