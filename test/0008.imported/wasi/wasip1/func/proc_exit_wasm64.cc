...
#include <uwvm2/imported/wasi/wasip1/func/proc_exit_wasm64.h>
...
int main()
{
    using ::uwvm2::imported::wasi::wasip1::abi::exitcode_wasm64_t;
    ...
    auto const code = static_cast<exitcode_wasm64_t>(expected_code);

    ::uwvm2::imported::wasi::wasip1::func::proc_exit_wasm64(env, code);

    if(!g_exit_called)
    {
        ::fast_io::io::perrln(::fast_io::u8err(), u8"proc_exit_wasm64: callback not invoked");
        ::fast_io::fast_terminate();
    }

    if(g_last_exit_code != expected_code)
    {
        ::fast_io::io::perrln(::fast_io::u8err(), u8"proc_exit_wasm64: exit code mismatch");
        ::fast_io::fast_terminate();
    }
}
