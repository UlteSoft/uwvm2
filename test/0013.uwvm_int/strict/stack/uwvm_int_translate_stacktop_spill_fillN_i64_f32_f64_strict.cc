...
namespace
{
    ...
    [[nodiscard]] byte_vec build_stacktop_spill_fillN_module()
    {
        module_builder mb{};
        ...
        // f1: i64 spillN+fillN
        // Push 3 i64 values, call nop, drop 1, add remaining two => 3.
        {
            func_type ty{{}, {k_val_i64}};
            func_body fb{};
            auto& c = fb.code;
            op(c, wasm_op::i64_const); i64(c, 1);
            op(c, wasm_op::i64_const); i64(c, 2);
            op(c, wasm_op::i64_const); i64(c, 3);
            op(c, wasm_op::i64_const); i64(c, 0);  // call param (dummy)
    ...
    [[nodiscard]] int test_translate_stacktop_spill_fillN_i64_f32_f64() noexcept
    {
        ...
        auto wasm = build_stacktop_spill_fillN_module();
        auto prep = prepare_runtime_from_wasm(wasm, u8"uwvm2test_stacktop_spill_fillN");
        ...
        {
            auto const& bc_i64 = cm.local_funcs.index_unchecked(1).op.operands;
            auto const& bc_f32 = cm.local_funcs.index_unchecked(2).op.operands;
            auto const& bc_f64 = cm.local_funcs.index_unchecked(3).op.operands;

            UWVM2TEST_REQUIRE((contains_stacktop_to_operand_stack_spillN<opt, wasm_i64>(bc_i64)));
            UWVM2TEST_REQUIRE((contains_operand_stack_to_stacktop_fillN<opt, wasm_i64>(bc_i64)));

            UWVM2TEST_REQUIRE((contains_stacktop_to_operand_stack_spillN<opt, wasm_f32>(bc_f32)));
            UWVM2TEST_REQUIRE((contains_operand_stack_to_stacktop_fillN<opt, wasm_f32>(bc_f32)));

            UWVM2TEST_REQUIRE((contains_stacktop_to_operand_stack_spillN<opt, wasm_f64>(bc_f64)));
            UWVM2TEST_REQUIRE((contains_operand_stack_to_stacktop_fillN<opt, wasm_f64>(bc_f64)));
        }
        ...
        // Semantics.
        {
            ...
            UWVM2TEST_REQUIRE(load_i64(rr1.results) == 3);
            ...
            UWVM2TEST_REQUIRE(load_f32(rr2.results) == 3.0f);
            ...
            UWVM2TEST_REQUIRE(load_f64(rr3.results) == 3.0);
        }
...
int main()
{
    return test_translate_stacktop_spill_fillN_i64_f32_f64();
}
