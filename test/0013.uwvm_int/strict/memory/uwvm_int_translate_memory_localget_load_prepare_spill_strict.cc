...
namespace
{
    ...
    [[nodiscard]] byte_vec build_memory_localget_load_prepare_spill_module()
    {
        module_builder mb{};
        mb.has_memory = true;
        mb.memory_min = 1u;
    ...
    [[nodiscard]] int check_bytecode(compiled_module_t const& cm, ::uwvm2::object::memory::linear::native_memory_t const& mem) noexcept
    {
        ...
        auto const exp_load = optable::translate::get_uwvmint_i32_load_localget_off_fptr_from_tuple<Opt>(curr, mem, tuple);
        auto const exp_load_add_imm = optable::translate::get_uwvmint_i32_load_add_imm_fptr_from_tuple<Opt>(curr, mem, tuple);
        auto const exp_load_and_imm = optable::translate::get_uwvmint_i32_load_and_imm_fptr_from_tuple<Opt>(curr, mem, tuple);
        ...
        UWVM2TEST_REQUIRE(bytecode_contains_fptr(bc0, exp_load));
        UWVM2TEST_REQUIRE(bytecode_contains_fptr(bc1, exp_load_add_imm));
        UWVM2TEST_REQUIRE(bytecode_contains_fptr(bc2, exp_load_and_imm));
        UWVM2TEST_REQUIRE(bytecode_contains_fptr(bc3, exp_load8_s));
        UWVM2TEST_REQUIRE(bytecode_contains_fptr(bc4, exp_load8_u));
    ...
    [[nodiscard]] int test_translate_memory_localget_load_prepare_spill() noexcept
    {
        ...
        auto wasm = build_memory_localget_load_prepare_spill_module();

        if(abi_mode_enabled("byref"))
        {
            constexpr auto opt{k_test_byref_opt};
            UWVM2TEST_REQUIRE(run_once<opt>(wasm, u8"uwvm2test_memory_localget_load_prepare_spill_byref", false) == 0);
        }

        if(abi_mode_enabled("tail-min"))
        {
            constexpr auto opt{k_test_tail_min_opt};
            UWVM2TEST_REQUIRE(run_once<opt>(wasm, u8"uwvm2test_memory_localget_load_prepare_spill_tailmin", false) == 0);
        }

        {
            constexpr auto opt{k_test_tail_i32_ring2_opt};
            static_assert(compiler::details::interpreter_tuple_has_no_holes<opt>());
            UWVM2TEST_REQUIRE(run_once<opt>(wasm, u8"uwvm2test_memory_localget_load_prepare_spill_ring2", true) == 0);
        }

        if(abi_mode_enabled("tail-sysv"))
        {
            constexpr auto opt{k_test_tail_sysv_opt};
            UWVM2TEST_REQUIRE(run_once<opt>(wasm, u8"uwvm2test_memory_localget_load_prepare_spill_tail_sysv", false) == 0);
        }

        if(abi_mode_enabled("tail-aapcs64"))
        {
            constexpr auto opt{k_test_tail_aapcs64_opt};
            UWVM2TEST_REQUIRE(run_once<opt>(wasm, u8"uwvm2test_memory_localget_load_prepare_spill_tail_aapcs64", false) == 0);
        }
...
int main()
{
    return test_translate_memory_localget_load_prepare_spill();
}
