...
namespace
{
    ...
    inline constexpr optable::uwvm_interpreter_translate_option_t k_test_tail_u16_pending_flush_ring2_opt{
        .is_tail_call = true,
        ...
        return bytecode_contains_any_i32_curr<Opt>(bc,
                                                   [&](optable::uwvm_interpreter_stacktop_currpos_t curr) noexcept
                                                   {
                                                       ...
                                                       else
                                                       {
                                                           return bytecode_contains_fptr(
                                                               bc,
                                                               optable::translate::get_uwvmint_i32_load16_u_fptr_from_tuple<Opt>(curr, tuple));
                                                       }
                                                   });
    }
    ...
    [[nodiscard]] byte_vec build_memory_u16_copy_scaled_index_pending_flush_bytecode_module()
    {
        module_builder mb{};
        mb.has_memory = true;
        mb.memory_min = 1u;
    ...
    [[nodiscard]] int compile_check_and_run(runtime_module_t const& rt, bool verify_bytecode) noexcept
    {
        ...
        if(verify_bytecode)
        {
            if constexpr(Opt.is_tail_call)
            {
                ...
                UWVM2TEST_REQUIRE(check_bytecode<Opt>(cm, ::std::addressof(mem)) == 0);
            }
            else
            {
                UWVM2TEST_REQUIRE(check_bytecode<Opt>(cm, nullptr) == 0);
            }
        }
    ...
    [[nodiscard]] int test_translate_memory_u16_copy_scaled_index_pending_flush_bytecode() noexcept
    {
        ...
        auto wasm = build_memory_u16_copy_scaled_index_pending_flush_bytecode_module();
        auto prep = prepare_runtime_from_wasm(wasm, u8"uwvm2test_memory_u16_copy_scaled_index_pending_flush_bytecode");
        ...
        static_assert(compiler::details::interpreter_tuple_has_no_holes<k_test_tail_u16_pending_flush_ring2_opt>());
        UWVM2TEST_REQUIRE(compile_check_and_run<k_test_tail_u16_pending_flush_ring2_opt>(rt, true) == 0);
...
int main()
{
    try
    {
        return test_translate_memory_u16_copy_scaled_index_pending_flush_bytecode();
    }
    catch(...)
    {
