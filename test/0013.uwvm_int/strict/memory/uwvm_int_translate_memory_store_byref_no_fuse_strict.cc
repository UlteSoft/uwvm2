...
namespace
{
    ...
    [[nodiscard]] byte_vec build_memory_store_byref_no_fuse_module()
    {
        module_builder mb{};
        mb.has_memory = true;
        mb.memory_min = 1u;
        ...
        // f0: local.get addr ; local.get v ; i64.store ; local.get addr ; i64.load
        {
            ...
            op(c, wasm_op::i64_store); u32(c, 3u); u32(c, 6u);

            op(c, wasm_op::local_get); u32(c, 0u);
            op(c, wasm_op::i64_load); u32(c, 3u); u32(c, 6u);
            op(c, wasm_op::end);

            (void)mb.add_func(::std::move(ty), ::std::move(fb));
        }

        // f1: local.get addr ; local.get v ; i32.store8 ; local.get addr ; i32.load8_u
        {
            ...
            op(c, wasm_op::local_get); u32(c, 0u);
            op(c, wasm_op::i32_load8_u); u32(c, 0u); u32(c, 5u);
            op(c, wasm_op::end);

            (void)mb.add_func(::std::move(ty), ::std::move(fb));
        }

        // f2: local.get addr ; i32.const imm ; i32.store8 ; local.get addr ; i32.load8_u
        {
            ...
            op(c, wasm_op::local_get); u32(c, 0u);
            op(c, wasm_op::i32_const); i32(c, 0x17c);
        ...
        // f3: local.get addr ; local.get v ; i32.store16 ; local.get addr ; i32.load16_u
        {
            func_type ty{{k_val_i32, k_val_i32}, {k_val_i32}};
            func_body fb{};
            auto& c = fb.code;
            ...
            op(c, wasm_op::local_get); u32(c, 0u);
            op(c, wasm_op::i32_load16_u); u32(c, 1u); u32(c, 10u);
            op(c, wasm_op::end);

            (void)mb.add_func(::std::move(ty), ::std::move(fb));
        }

        // f4: local.get addr ; i32.const imm ; i32.store16 ; local.get addr ; i32.load16_u
        {
            func_type ty{{k_val_i32}, {k_val_i32}};
            func_body fb{};
            auto& c = fb.code;
            ...
            op(c, wasm_op::local_get); u32(c, 0u);
            op(c, wasm_op::i32_const); i32(c, 0x1beef);
    ...
    [[nodiscard]] int run_suite(compiled_module_t const& cm, runtime_module_t const& rt) noexcept
    {
...

        {
            auto rr = Runner::run(cm.local_funcs.index_unchecked(0),
                                  ...
                                  pack_i32_i64(24, static_cast<::std::int64_t>(0x1122334455667788ull)),
                                  nullptr,
                                  nullptr);
...

        {
            auto rr = Runner::run(cm.local_funcs.index_unchecked(1),
                                  ...
                                  pack_i32x2(40, static_cast<::std::int32_t>(0x1abu)),
                                  nullptr,
                                  nullptr);
            UWVM2TEST_REQUIRE(static_cast<::std::uint32_t>(load_i32(rr.results)) == 0xabu);
        }

        {
            auto rr = Runner::run(cm.local_funcs.index_unchecked(2),
                                  ...
                                  nullptr,
                                  nullptr);
            UWVM2TEST_REQUIRE(static_cast<::std::uint32_t>(load_i32(rr.results)) == 0x7cu);
        }

        {
            auto rr = Runner::run(cm.local_funcs.index_unchecked(3),
                                  rt.local_defined_function_vec_storage.index_unchecked(3),
                                  pack_i32x2(64, static_cast<::std::int32_t>(0x12345678u)),
                                  nullptr,
                                  nullptr);
            UWVM2TEST_REQUIRE(static_cast<::std::uint32_t>(load_i32(rr.results)) == 0x5678u);
        }

        {
            auto rr = Runner::run(cm.local_funcs.index_unchecked(4),
                                  ...
                                  nullptr,
                                  nullptr);
            UWVM2TEST_REQUIRE(static_cast<::std::uint32_t>(load_i32(rr.results)) == 0xbeefu);
        }
    ...
    [[nodiscard]] int check_byref_bytecode(compiled_module_t const& cm) noexcept
    {
        ...
        auto const exp_i64_store_plain = optable::translate::get_uwvmint_i64_store_fptr_from_tuple<Opt>(curr, tuple);
        ...
        UWVM2TEST_REQUIRE(bytecode_contains_fptr(bc2, exp_i32_const));
        UWVM2TEST_REQUIRE(bytecode_contains_fptr(bc2, exp_store8_plain));

        UWVM2TEST_REQUIRE(bytecode_contains_fptr(bc3, exp_local_get_i32));
        UWVM2TEST_REQUIRE(bytecode_contains_fptr(bc3, exp_store16_plain));

        UWVM2TEST_REQUIRE(bytecode_contains_fptr(bc4, exp_local_get_i32));
        UWVM2TEST_REQUIRE(bytecode_contains_fptr(bc4, exp_i32_const));
        UWVM2TEST_REQUIRE(bytecode_contains_fptr(bc4, exp_store16_plain));
        return 0;
    }
    ...
    [[nodiscard]] int test_translate_memory_store_byref_no_fuse() noexcept
    {
        ...
        auto wasm = build_memory_store_byref_no_fuse_module();
        auto prep = prepare_runtime_from_wasm(wasm, u8"uwvm2test_memory_store_byref_no_fuse");
...
int main()
{
    try
    {
        return test_translate_memory_store_byref_no_fuse();
    }
    catch(...)
    {
