...
namespace
{
    ...
    [[nodiscard]] byte_vec build_memory_scalar4_merged_i64_store32_local_plus_imm_no_fuse_module()
    {
        module_builder mb{};
        mb.has_memory = true;
        mb.memory_min = 1u;
...

        {
            ...
            op(c, wasm_op::i32_const); i32(c, 116);
            op(c, wasm_op::i64_load32_u); u32(c, 2u); u32(c, 0u);
            op(c, wasm_op::end);
            (void)mb.add_func(::std::move(ty), ::std::move(fb));
        }
...

        {
            ...
            op(c, wasm_op::i64_store32); u32(c, 2u); u32(c, 1u);
                                     ...
                                     ::uwvm2::object::memory::linear::native_memory_t const& mem) noexcept
    {
        ...
        auto require_store32 = [&](auto const& bc) noexcept -> int
        {
        ...
        UWVM2TEST_REQUIRE(require_store32(bc0) == 0);
        UWVM2TEST_REQUIRE(require_store32(bc1) == 0);
        UWVM2TEST_REQUIRE(require_store32(bc2) == 0);
        UWVM2TEST_REQUIRE(require_store32(bc3) == 0);
        return 0;
    }
#endif
    ...
    [[nodiscard]] int test_translate_memory_scalar4_merged_i64_store32_local_plus_imm_no_fuse() noexcept
    {
        ...
        auto wasm = build_memory_scalar4_merged_i64_store32_local_plus_imm_no_fuse_module();
        auto prep = prepare_runtime_from_wasm(wasm, u8"uwvm2test_memory_scalar4_merged_i64_store32_local_plus_imm_no_fuse");
...
int main()
{
    try
    {
        return test_translate_memory_scalar4_merged_i64_store32_local_plus_imm_no_fuse();
    }
    catch(...)
    {
