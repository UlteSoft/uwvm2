...
namespace
{
    ...
    [[nodiscard]] bool contains_any_local_get_i32(ByteStorage const& bc) noexcept
    {
        ...
        for(::std::size_t i32_pos{Opt.i32_stack_top_begin_pos}; i32_pos != Opt.i32_stack_top_end_pos; ++i32_pos)
        {
            auto curr = make_initial_stacktop_currpos<Opt>();
            curr.i32_stack_top_curr_pos = i32_pos;
            if(bytecode_contains_fptr(bc, optable::translate::get_uwvmint_local_get_i32_fptr_from_tuple<Opt>(curr, tuple))) { return true; }
        }
        return false;
    }
    ...
    [[nodiscard]] bool contains_any_local_get_i64(ByteStorage const& bc) noexcept
    {
        ...
        for(::std::size_t i64_pos{Opt.i64_stack_top_begin_pos}; i64_pos != Opt.i64_stack_top_end_pos; ++i64_pos)
        {
            auto curr = make_initial_stacktop_currpos<Opt>();
            curr.i64_stack_top_curr_pos = i64_pos;
            if(bytecode_contains_fptr(bc, optable::translate::get_uwvmint_local_get_i64_fptr_from_tuple<Opt>(curr, tuple))) { return true; }
        }
        return false;
    }

    [[nodiscard]] byte_vec build_memory_scalar4_merged_i64_store32_localget_fuse_module()
    {
        module_builder mb{};
        mb.has_memory = true;
        mb.memory_min = 1u;
...

        {
            ...
            op(c, wasm_op::i32_const); i32(c, 108);
            op(c, wasm_op::i64_load32_u); u32(c, 2u); u32(c, 0u);
            op(c, wasm_op::end);
            (void)mb.add_func(::std::move(ty), ::std::move(fb));
        }

        {
            ...
            op(c, wasm_op::i32_const); i32(c, 148);
                                     ...
                                     ::uwvm2::object::memory::linear::native_memory_t const& mem) noexcept
    {
        ...
        auto require_fused = [&](auto const& bc, bool allow_local_get_i64) noexcept -> int
        {
            ...
            UWVM2TEST_REQUIRE(!contains_any_local_get_i32<Opt>(bc));
            if(allow_local_get_i64)
            {
                UWVM2TEST_REQUIRE(contains_any_local_get_i64<Opt>(bc));
            }
            else
            {
                UWVM2TEST_REQUIRE(!contains_any_local_get_i64<Opt>(bc));
        ...
        UWVM2TEST_REQUIRE(require_fused(bc0, false) == 0);
        UWVM2TEST_REQUIRE(require_fused(bc1, false) == 0);
        UWVM2TEST_REQUIRE(require_fused(bc2, false) == 0);
        UWVM2TEST_REQUIRE(require_fused(bc3, true) == 0);
        return 0;
    }
#endif
    ...
    [[nodiscard]] int run_suite(compiled_module_t const& cm, runtime_module_t const& rt) noexcept
    {
        ...
        UWVM2TEST_REQUIRE(load_i64(Runner::run(cm.local_funcs.index_unchecked(2),
                                              ...
                                              pack_i32_i64(128, 0x80000010ll),
                                              nullptr,
                                              nullptr)
                                       .results) == -2147483632ll);

        UWVM2TEST_REQUIRE(load_i64(Runner::run(cm.local_funcs.index_unchecked(3),
                                              rt.local_defined_function_vec_storage.index_unchecked(3),
                                              pack_i64_i32_i64(9, 164, 0x7ffffffe),
                                              nullptr,
                                              nullptr)
                                       .results) == 2147483655ll);
        return 0;
    }
    ...
    [[nodiscard]] int test_translate_memory_scalar4_merged_i64_store32_localget_fuse() noexcept
    {
        ...
        auto wasm = build_memory_scalar4_merged_i64_store32_localget_fuse_module();
        auto prep = prepare_runtime_from_wasm(wasm, u8"uwvm2test_memory_scalar4_merged_i64_store32_localget_fuse");
...
int main()
{
    try
    {
        return test_translate_memory_scalar4_merged_i64_store32_localget_fuse();
    }
    catch(...)
    {
