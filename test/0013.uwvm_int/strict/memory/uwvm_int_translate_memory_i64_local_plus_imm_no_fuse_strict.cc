...
namespace
{
    ...
    [[nodiscard]] byte_vec build_memory_i64_local_plus_imm_no_fuse_module()
    {
        module_builder mb{};
        mb.has_memory = true;
        mb.memory_min = 1u;
        ...
        // f0: local.get base ; i32.const 4 ; i32.add ; i64.load
        {
        ...
        // f1: local.get base ; i32.const 8 ; i32.add ; local.get v ; i64.store ; reload same addr
        {
        ...
        // f2: local.get base ; i32.const 12 ; i32.add ; local.get v ; i64.store32 ; reload with i64.load32_u
        {
            func_type ty{{k_val_i32, k_val_i64}, {k_val_i64}};
            func_body fb{};
            auto& c = fb.code;
    ...
    [[nodiscard]] int run_suite(compiled_module_t const& cm, runtime_module_t const& rt) noexcept
    {
...

        {
            auto rr = Runner::run(cm.local_funcs.index_unchecked(1),
                                  ...
                                  pack_i32_i64(80, static_cast<::std::int64_t>(0x0102030405060708ull)),
                                  nullptr,
                                  nullptr);
...

        {
            auto rr = Runner::run(cm.local_funcs.index_unchecked(2),
                                  ...
                                  pack_i32_i64(96, static_cast<::std::int64_t>(0x1122334455667788ull)),
                                  nullptr,
                                  nullptr);
            UWVM2TEST_REQUIRE(static_cast<::std::uint64_t>(load_i64(rr.results)) == 0x55667788ull);
        }
    ...
    [[nodiscard]] int check_byref_bytecode(compiled_module_t const& cm) noexcept
    {
        ...
        UWVM2TEST_REQUIRE(bytecode_contains_fptr(bc0, exp_i32_const));
        UWVM2TEST_REQUIRE(bytecode_contains_fptr(bc0, exp_i32_add));
        UWVM2TEST_REQUIRE(bytecode_contains_fptr(bc0, exp_i64_load_plain));
    ...
    [[nodiscard]] int test_translate_memory_i64_local_plus_imm_no_fuse() noexcept
    {
        ...
        auto wasm = build_memory_i64_local_plus_imm_no_fuse_module();
        auto prep = prepare_runtime_from_wasm(wasm, u8"uwvm2test_memory_i64_local_plus_imm_no_fuse");
...
int main()
{
    try
    {
        return test_translate_memory_i64_local_plus_imm_no_fuse();
    }
    catch(...)
    {
