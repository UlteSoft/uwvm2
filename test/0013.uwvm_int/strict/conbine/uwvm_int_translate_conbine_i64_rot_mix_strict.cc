...
namespace
{
    ...
    [[nodiscard]] byte_vec build_conbine_i64_rot_mix_module()
    {
        module_builder mb{};
        ...
        // f0: (rotl(x, 5) ^ y) + 11
        {
            ...
            func_body fb{};
            auto& c = fb.code;
            op(c, wasm_op::local_get); u32(c, 0u);  // x
            op(c, wasm_op::i64_const); i64(c, 5);
            op(c, wasm_op::i64_rotl);
            op(c, wasm_op::local_get); u32(c, 1u);  // y
        ...
        // f1: y = y ^ rotl(x, 5) ; return y (local.set)
        {
            func_type ty{{k_val_i64, k_val_i64}, {k_val_i64}};
            func_body fb{};
            auto& c = fb.code;
            op(c, wasm_op::local_get); u32(c, 0u);  // y
            op(c, wasm_op::local_get); u32(c, 1u);  // x
            op(c, wasm_op::i64_const); i64(c, 5);
            op(c, wasm_op::i64_rotl);
            op(c, wasm_op::i64_xor);
        ...
        // f2: y = y ^ rotl(x, 5) ; return y (local.tee)
        {
            func_type ty{{k_val_i64, k_val_i64}, {k_val_i64}};
            func_body fb{};
            auto& c = fb.code;
        ...
        // f3: flush after `rot_xor_add_i64_after_rotl`
        {
        ...
        // f4: flush after `rot_xor_add_i64_after_gety`
        {
            func_type ty{{k_val_i64, k_val_i64}, {k_val_i64}};
            func_body fb{};
            auto& c = fb.code;
        ...
        // f5: flush after `rot_xor_add_i64_after_xor`
        {
        ...
        // f6: flush after `rotl_xor_local_set_i64_after_xor`
        {
            func_type ty{{k_val_i64, k_val_i64}, {k_val_i32}};
            func_body fb{};
            auto& c = fb.code;
...
# if defined(UWVM_ENABLE_UWVM_INT_EXTRA_HEAVY_COMBINE_OPS)
            ...
            UWVM2TEST_REQUIRE(!bytecode_contains_i64_variant<Opt>(cm.local_funcs.index_unchecked(6).op.operands, make_rotl_xor_tee));
# endif

            UWVM2TEST_REQUIRE(!bytecode_contains_i64_variant<Opt>(cm.local_funcs.index_unchecked(3).op.operands, make_rot_xor_add));
            UWVM2TEST_REQUIRE(!bytecode_contains_i64_variant<Opt>(cm.local_funcs.index_unchecked(4).op.operands, make_rot_xor_add));
            UWVM2TEST_REQUIRE(!bytecode_contains_i64_variant<Opt>(cm.local_funcs.index_unchecked(5).op.operands, make_rot_xor_add));
#endif
        }

        using Runner = interpreter_runner<Opt>;

        auto run_i64_1 = [&](::std::size_t fn_index, ::std::uint64_t a) noexcept -> ::std::uint64_t
        {
            auto rr = Runner::run(cm.local_funcs.index_unchecked(fn_index),
                                  rt.local_defined_function_vec_storage.index_unchecked(fn_index),
                                  pack_i64(::std::bit_cast<::std::int64_t>(a)),
                                  nullptr,
                                  nullptr);
            return static_cast<::std::uint64_t>(load_i64(rr.results));
        };

        auto run_i64_2 = [&](::std::size_t fn_index, ::std::uint64_t a, ::std::uint64_t b) noexcept -> ::std::uint64_t
        {
            auto rr = Runner::run(cm.local_funcs.index_unchecked(fn_index),
                                  rt.local_defined_function_vec_storage.index_unchecked(fn_index),
                                  pack_i64x2(::std::bit_cast<::std::int64_t>(a), ::std::bit_cast<::std::int64_t>(b)),
                                  nullptr,
                                  nullptr);
            return static_cast<::std::uint64_t>(load_i64(rr.results));
        };

        auto run_i32_2 = [&](::std::size_t fn_index, ::std::uint64_t a, ::std::uint64_t b) noexcept -> ::std::int32_t
        {
            auto rr = Runner::run(cm.local_funcs.index_unchecked(fn_index),
                                  rt.local_defined_function_vec_storage.index_unchecked(fn_index),
                                  pack_i64x2(::std::bit_cast<::std::int64_t>(a), ::std::bit_cast<::std::int64_t>(b)),
        ...
        {
            UWVM2TEST_REQUIRE(run_i64_2(0uz, x, y) == static_cast<::std::uint64_t>((rotl64(x, 5u) ^ y) + 11ull));
            UWVM2TEST_REQUIRE(run_i64_1(3uz, x) == static_cast<::std::uint64_t>(rotl64(x, 5u) + 1ull));
            UWVM2TEST_REQUIRE(run_i64_2(4uz, x, y) == static_cast<::std::uint64_t>(rotl64(x, 7u) + y));

            ::std::uint64_t const xored{rotl64(x, 13u) ^ y};
            UWVM2TEST_REQUIRE(run_i32_2(5uz, x, y) == (xored == 0u ? 1 : 0));
        }
    ...
    [[nodiscard]] int test_translate_conbine_i64_rot_mix() noexcept
    {
        ...
        auto wasm = build_conbine_i64_rot_mix_module();
        auto prep = prepare_runtime_from_wasm(wasm, u8"uwvm2test_conbine_i64_rot_mix");
...
int main()
{
    try
    {
        return test_translate_conbine_i64_rot_mix();
    }
    catch(...)
    {
