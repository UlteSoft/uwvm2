...
namespace
{
    ...
    [[nodiscard]] byte_vec build_loop_f64_loop_run_module()
    {
        module_builder mb{};
        ...
        // f0: matches the extra-heavy `loop_f64` mega-fusion pattern (when enabled).
        // Returns: x + f64.convert_i32_u(i), so both hot locals remain observable.
        {
            ...
            fb.locals.push_back({1u, k_val_f64});  // local1 x
            auto& c = fb.code;
    ...
    [[nodiscard]] double expected_f0() noexcept
    {
        ...
        ::std::uint32_t i = 0u;
        wasm_f64 x = static_cast<wasm_f64>(0.0);
        ...
        return num::eval_float_binop<num::float_binop::add, wasm_f64>(x, static_cast<wasm_f64>(i));
    }

    template <optable::uwvm_interpreter_translate_option_t Opt>
    [[nodiscard]] int run_loop_f64_loop_run_suite(runtime_module_t const& rt, double expected) noexcept
    {
    ...
    [[nodiscard]] int test_translate_loop_f64_loop_run() noexcept
    {
        ...
        auto wasm = build_loop_f64_loop_run_module();
        auto prep = prepare_runtime_from_wasm(wasm, u8"uwvm2test_loop_f64_loop_run");
...

        {
            ...
            constexpr auto exp_loop_run =
                optable::translate::get_uwvmint_loop_f64_loop_run_fptr_from_tuple<opt>(curr, tuple);
...

        {
            ...
            UWVM2TEST_REQUIRE(run_loop_f64_loop_run_suite<opt>(rt, expected) == 0);
        }
...
int main()
{
    return test_translate_loop_f64_loop_run();
}
