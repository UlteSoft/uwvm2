...
namespace
{
    ...
    [[nodiscard]] byte_vec build_f32_inv_cube_sum_loop_run_module()
    {
        module_builder mb{};
        ...
        // f0: matches the extra-heavy test9 f32 inv-cube sum-loop mega-fusion pattern (when enabled).
    ...
    [[nodiscard]] float expected_f0() noexcept
    {
        ...
        for(;;)
        {
            ...
            ::std::uint32_t const denom0_u = static_cast<::std::uint32_t>(static_cast<::std::uint32_t>(i_u * i_u) * i_u);
            ::std::uint32_t const denom1_u = static_cast<::std::uint32_t>(static_cast<::std::uint32_t>(im1_u * im1_u) * im1_u);
    ...
    [[nodiscard]] int run_f32_inv_cube_sum_loop_run_suite(runtime_module_t const& rt, float expected) noexcept
    {
    ...
    [[nodiscard]] int test_translate_f32_inv_cube_sum_loop_run() noexcept
    {
        ...
        auto wasm = build_f32_inv_cube_sum_loop_run_module();
        auto prep = prepare_runtime_from_wasm(wasm, u8"uwvm2test_f32_inv_cube_sum_loop_run");
        ...
        // Tailcall mode: bytecode should contain the mega-op when extra-heavy combine is enabled.
        {
            ...
            constexpr auto exp_loop_run =
                optable::translate::get_uwvmint_f32_inv_cube_sum_loop_run_fptr_from_tuple<opt>(curr, tuple);
        ...
        // Byref mode: semantics smoke.
        {
            ...
            UWVM2TEST_REQUIRE(run_f32_inv_cube_sum_loop_run_suite<opt>(rt, expected) == 0);
        }
...
int main()
{
    return test_translate_f32_inv_cube_sum_loop_run();
}
