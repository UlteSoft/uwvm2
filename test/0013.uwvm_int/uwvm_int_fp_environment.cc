// Exercise ROS production full execution, both with and without LLVM and both TLS representations.
// ROS intentionally has no lazy or tiered modes; do not restore the deleted runtime surfaces.
...
namespace
{
    ...
    [[nodiscard]] bool check_entry(unsigned function_index, Float left, Float right, Bits expected)
    {
        ...
        runtime::full_compile_and_run_main_module(u8"fp-main", {function_index, buffers});
        auto const actual{::std::bit_cast<Bits>(result)};
        if(actual == expected) { return true; }
        ::std::fprintf(stderr, "uwvm_int_fp_environment: function=%u expected=%llx actual=%llx\n",
                       function_index, static_cast<unsigned long long>(expected), static_cast<unsigned long long>(actual));
        return false;
    }

    [[nodiscard]] int run_suite(mode::runtime_compiler_t backend)
    {
            ...
            for(unsigned index{}; index != 16u; ++index)
            {
                ...
                if(index < 8u)
                {
                    ok = check_entry(index + 1u, multiply ? ::std::numeric_limits<float>::min() : 1.0f,
                                     ...
                                     ::std::uint32_t{multiply ? 0x00400000u : 0x3f800000u});
                }
                else
                {
                    ok = check_entry(index + 1u, multiply ? ::std::numeric_limits<double>::min() : 1.0,
                                     ...
                                     ::std::uint64_t{multiply ? 0x0008000000000000ull : 0x3ff0000000000001ull});
                }
...
# endif
        ...
        runtime::full_compile_and_run_main_module(u8"fp-main", {9u, narrow_buffers});
# if defined(__m68k__)
...
#endif
    if(auto const result{run_suite(mode::runtime_compiler_t::uwvm_interpreter_only)}; result != 0) { return result; }
#if defined(UWVM_RUNTIME_LLVM_JIT)
    if(auto const result{run_suite(mode::runtime_compiler_t::llvm_jit_only)}; result != 0) { return result; }
#endif
    return 0;
}
