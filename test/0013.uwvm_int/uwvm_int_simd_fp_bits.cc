...
UWVM_NOINLINE unsigned check_simd(UInt input)
{
    ...
    unsigned errors{};
    for(unsigned op = 0; op != 9; ++op) for(std::size_t lane = 0; lane != count; ++lane)
    {
        if(sizeof(Float) != 4 && op >= 7) continue;
        s::lane_array<UInt, count> left{}, right{};
        for(std::size_t i = 0; i != count; ++i) { left.lane[i] = input; right.lane[i] = UInt{}; }
        ...
        if(op == 2 || op == 3) { std::memcpy(sp, &r, sizeof(r)); sp += sizeof(r); }
        if(op == 4 || op == 7) { std::memcpy(values, &input, sizeof(input)); sp = values + sizeof(input); }
        if(op == 5 || op == 6) { *immediate++ = std::byte(lane); }
        if(op == 6) { std::memcpy(sp, &input, sizeof(input)); sp += sizeof(input); }
        if constexpr(Tail) { auto end = &finish_simd; std::memcpy(immediate, &end, sizeof(end)); }
#define CASES(SHAPE) \
...
#undef CASES
        ...
        else infinity = UInt{0x7ff0000000000000ull};
        if((op == 2 || op == 3) && magnitude != 0 && magnitude <= infinity &&
           (op == 2 ? (input & sign) == 0 : (input & sign) != 0)) expected = 0;
        if(op == 5 || op == 8)
        {
            UInt actual; std::memcpy(&actual, values, sizeof(actual));
        ...
        else
        {
            ...
            for(auto value : actual.lane) if(value != expected) ++errors;
        }
        if(errors) { std::fprintf(stderr, "SIMD tail=%d f%zu op=%u lane=%zu input=%016" PRIx64 "\n", Tail, sizeof(UInt)*8, op, lane, std::uint64_t(input)); return errors; }
    }
    return errors;
}
...
int main()
{
    ...
    { errors += check_simd<false, s::wasm_f32>(value); errors += check_simd<true, s::wasm_f32>(value); }
    for(std::uint64_t value : {0ull, 0x8000000000000000ull, 1ull, 0x8000000000000001ull, 0x7ff0000000000001ull, 0xfff0000000000123ull})
    { errors += check_simd<false, s::wasm_f64>(value); errors += check_simd<true, s::wasm_f64>(value); }
