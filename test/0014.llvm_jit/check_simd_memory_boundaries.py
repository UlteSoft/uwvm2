#!/usr/bin/env python3
"""Run exact-width SIMD accesses against real guard pages, in full and lazy LLVM modes.
...
def main():
    ...
    modes = {
        "full-o3": ["-Rcm", "full", "-Rllvm-full-policy", "pb-o3"],
        "full-debug": ["-Rcm", "full", "-Rllvm-full-policy", "debug"],
        "lazy-balanced": ["-Rcm", "lazy", "-Rllvm-lazy-policy", "balanced"],
        "lazy-debug": ["-Rcm", "lazy", "-Rllvm-lazy-policy", "debug"],
    }
    if args.jobs < 1:
    ...
    for name, wat, trap in fixtures():
        ...
        for mode, options in modes.items():
            command = [str(args.uwvm.resolve()), "-Rcc", "jit", "-Rct", "0", *options,
