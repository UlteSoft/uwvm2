...
#define LLVM_IMPORTED_BULK_REQUIRE(condition, message) \
    ...
    [[nodiscard]] strict::byte_vec build_imported_bulk_memory_module()
    {
        ...
        module.passive_datas.push_back(strict::passive_data_segment{.bytes = strict::byte_vec{
                                                                        ::std::byte{10},
                                                                        ::std::byte{20},
                                                                        ::std::byte{30},
                                                                        ::std::byte{40},
...

        {
            ...
            op(code, strict::wasm_op::i32_const); i32(code, 30000);
            op(code, strict::wasm_op::i32_const); i32(code, 1);
            op(code, strict::wasm_op::i32_const); i32(code, 4);
            ext(code, strict::wasm1p1_numeric_op::memory_init);
            strict::append_u32_leb(code, 0u);
            strict::append_u8(code, 0u);
            ext(code, strict::wasm1p1_numeric_op::data_drop);
            strict::append_u32_leb(code, 0u);
        ...
        {
            // A zero-length memory.init remains valid after data.drop when both offsets equal their respective bounds.
            strict::func_type type{{}, {}};
            strict::func_body body{};
            auto& code{body.code};
            op(code, strict::wasm_op::i32_const); i32(code, 65536);
    ...
    [[nodiscard]] int test_imported_bulk_memory()
    {
        ...
        LLVM_IMPORTED_BULK_REQUIRE(compiled.local_funcs.size() == 2uz, "unexpected local function count");
        ...
        LLVM_IMPORTED_BULK_REQUIRE(
            module_references_bridge<llvm_jit::details::llvm_jit_local_imported_memory_init_bridge>(*compiled.llvm_jit_module.llvm_module),
            "emitted IR does not reference the local-imported memory.init bridge");
        ...
        constexpr unsigned char initialized_bytes[]{20u, 30u, 40u, 50u};
        for(::std::size_t index{}; index != ::std::size(initialized_bytes); ++index)
        {
            expected[30000uz + index] = static_cast<::std::byte>(initialized_bytes[index]);
        }
        ...
        LLVM_IMPORTED_BULK_REQUIRE(
            strict::run_in_child_expect_trap_message("memory access out of bounds",
                                                     [&]
                                                     {
                                                         llvm_jit::details::llvm_jit_local_imported_memory_init_bridge(
                                                             reinterpret_cast<::std::uintptr_t>(provider),
                                                             0uz,
                                                             reinterpret_cast<::std::uintptr_t>(prepared.mod),
                                                             0u,
                                                             0,
                                                             0,
                                                             1);
                                                     }) == 0,
            "memory.init from a dropped data instance did not trap");
#endif
        return 0;
    }
}  // namespace
