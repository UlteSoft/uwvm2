...
def main():
    ...
    p.add_argument('--llc', type=Path, help='Code generator under test (ROS: bundled build/bin/llc)')
    p.add_argument('--products', nargs='+', choices=['full', 'ros'], default=['full', 'ros'])
    p.add_argument('--unconstrained', action='store_true', help='Test the upstream round-trip combine without the runtime workaround')
    ...
    for product, probe in [('full', a.full_probe.resolve()), ('ros', a.ros_probe.resolve())]:
        ...
        for variant, triple, cpu, features, emulator in variants:
            ...
            ir.write_bytes(run([probe, 'emit-raw' if a.unconstrained else 'emit', triple, features], f'{product}-{variant}-emit'))
            for opt in [0, 3]:
                stem = f'{product}-{variant}-O{opt}'
                obj, binary = out/(stem+'.o'), out/stem
                run([a.llc or sdk/'bin/llc', f'-O{opt}', '-mtriple='+triple, '-mcpu='+cpu, '-mattr='+features,
