...
namespace
{
    ...
    [[nodiscard]] feature_parameter make_wasm2_feature_parameter() noexcept
    {
        ...
        parameter.cli_mode = ::uwvm2::parser::wasm::standard::wasm1p1::features::wasm_feature_cli_mode::direct_wasm2;
        parameter.explicit_feature_wasm2 = true;
        parameter.disable_table_instructions = false;
        parameter.disable_multiple_tables = false;
        return policy;
    }
                                         ...
                                         ::std::string_view const backend)
    {
        ...
        auto command{quote(uwvm_path) + (backend == "jit" ? " -Raot" : " -Rint")};
...
int main(int argc, char** argv)
{
    ...
    for(::std::size_t case_index{}; case_index != ::std::size(cases); ++case_index)
    {
        ...
        auto const all_enabled{make_wasm2_feature_parameter()};
...
#endif
        if(!same_feature_diagnostic(standard_error, llvm_error))
        {
            ::std::fprintf(stderr, "standard error=%u LLVM error=%u\n",
                           static_cast<unsigned>(standard_error.err_code), static_cast<unsigned>(llvm_error.err_code));
            if(standard_error.err_code == error_code::wasm2_feature_required && llvm_error.err_code == error_code::wasm2_feature_required)
            {
                ::std::fprintf(stderr, "standard instruction=%u LLVM instruction=%u\n",
                               static_cast<unsigned>(standard_error.err_selectable.wasm2_feature_required.value),
                               static_cast<unsigned>(llvm_error.err_selectable.wasm2_feature_required.value));
            }
            return fail(case_index, "LLVM feature diagnostic differs from the standard validator");
        }
    ...
    ::uwvm2::parser::wasm::standard::wasm1p1::features::get_wasm1p1_parameter(mvp_policy).cli_mode =
        ::uwvm2::parser::wasm::standard::wasm1p1::features::wasm_feature_cli_mode::direct_mvp;
    if(validate_call_indirect_encoding(call_indirect_reserved_zero_module,
                                       sizeof(call_indirect_reserved_zero_module),
                                       mvp_policy,
                                       error_code::ok,
    ...
    auto const wasm2_policy{make_wasm2_feature_parameter()};
    if(validate_call_indirect_encoding(call_indirect_nonminimal_zero_module,
                                       sizeof(call_indirect_nonminimal_zero_module),
                                       wasm2_policy,
                                       error_code::ok,
