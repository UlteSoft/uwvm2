...
namespace
{
    ...
    char const module_name_str[] = "dl.example";
    ...
    char const* delivery_state_name(unsigned state) noexcept
    {
        switch(state)
        {
    ...
    void inspect_memory_impl(unsigned char* res_bytes, unsigned char* para_bytes)
    {
        ...
        std::fprintf(stderr,
                     "example-dl-cxx: state=%s backend=%u bytes=[%u,%u,%u,%u] sum=%d\n",
    ...
    void probe_host_apis_impl(unsigned char* res_bytes, unsigned char* para_bytes)
    {
        ...
        std::fprintf(stderr,
                     "example-dl-cxx: wasi argc=%" PRIuLEAST32 " argv_buf_size=%" PRIuLEAST32 "\n",
                     argc_value,
                     argv_buf_size_value);
        std::fflush(stderr);
...
extern "C" void uwvm_set_wasip1_host_api_v1(uwvm_wasip1_host_api_v1 const* api)
{
